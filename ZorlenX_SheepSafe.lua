...
function sheepSafe:SheepSafe()
   ...
   if (self.ccSlot == nil) then
      sheepSafe:p("Can't find "..self.cc.." on your action bar.	 Please	add it.")
      return false
   end

   if (not UnitName("target")) then
      sheepSafe:p("No target.")
      return false
   end
  ...
  if ZorlenX_IsCrowdControlled() then
   ...
   if (not UnitCanAttack("player", "target")) then
      sheepSafe:p("Can't attack	that target.")
      return false
   end

   if (UnitHealth("target") == 0) then
      sheepSafe:p("Can't "..self.ccverb2.." target, target is dead.")
      return false
   end
   ...
   if (not string.find(self.validtargets, creatureType)) then
	sheepSafe:p("Can't "..self.ccverb2.." that target: not "..self.validtargetsdesc)
    return false
   end

   local isUsable, notEnoughMana = IsUsableAction(self.ccSlot)
   if (isUsable	~= 1) then
      sheepSafe:p(self.cc.." not ready.")
      return false
   end

   if (IsActionInRange(self.ccSlot) == 0) then
      sheepSafe:p(self.cc.." not in range.")
      return false
   end

   local start,	duration, enable = GetActionCooldown(self.ccSlot)
   if (duration	~= 0) then
      sheepSafe:p(self.cc.." not ready,	in cooldown.")
      return false
   end
   ...
   if tmpp then
    sheepSafe:d("pretarget spell: "..tmpp)
   end
   sheepSafe:d("pretarget cast:"..sheepSafe:BtoS(self.preTargetSpellCast))
